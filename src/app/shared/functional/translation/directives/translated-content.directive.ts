...
export class TranslatedContentDirective implements OnInit, OnDestroy, AfterContentInit {
  ...
  public ngOnInit(): void {
    this.rawTranslation = merge(
      ...
      this.translateService.onLangChange
        .asObservable()
        .pipe(switchMap(() => this.translateService.get(this.translationKey))),
    );
  }
  ...
  public ngAfterContentInit(): void {
    ...
    this.translationData = combineLatest(this.rawTranslation, elementsChanges).pipe(
      map(([rawTranslation]) => {
        return {
  ...
  private render(translationData: TranslationData): void {
    ...
    while (lastTokenEnd < translationData.rawTranslation.length) {
      ...
      if (embeddedElementTemplate) {
        const embeddedElementView = embeddedElementTemplate.viewRef.createEmbeddedView(
          embeddedElementTemplate.templateRef,
        );
